...
2. Set these variables in `backend/saasapp-restapi/.env`:
    - `SMTP_HOST=localhost`
    - `SMTP_PORT=1025`
    - `MAIL_USERNAME=`
    - `MAIL_PASSWORD=`
    - `NO_REPLY_EMAIL=no-reply@saasapp.local`
...
3. Set these variables in `backend/saasapp-restapi/.env`:
